...
- **しかし利用者が手動で正しい税率を選ぶ必要があり、ミスや漏れの温床**。発効日付き税率マスタの導入を強く推奨。
- **インボイス経過措置控除率の段階変更（2026/10/01の80→70%）は、ほぼ確実に対応必須**。新規テーブル追加が必要。
- **食料品ゼロ税率は、`numRateConsumptionTax` のラベル拡張＋食料品判別ロジック追加で対応可能**。法案確定後に着手。

新ドメイン層の `ConsumptionTax/` 配下に `TaxRateRegistry` と `Partner/` サブドメインを追加することで、**期中変更を含む将来の制度変更への耐性**を確保する。
