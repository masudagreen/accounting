 ...
 * http://rucaro.org/
 */
class Code_Else_Plugin_Accounting_Jpn_LogSearch extends Code_Else_Plugin_Accounting_Jpn_Log
{
	protected $_childSelf = array(
		'pathTplJs'   => 'else/plugin/accounting/js/jpn/logSearch.js',
		'pathVarsJs'  => 'back/tpl/vars/else/plugin/accounting/<strLang>/js/jpn/logSearch.php',
	);
	...
	protected function _iniJs()
	{
		$this->_setJs(array(
			...
			'strColumnSearch'   => 'jsonLogNaviSearch',
			'flagEntitySearch'  => 1,
			'flagAccountSearch' => 1,
			'arrSearch'       => array(),
		));
	...
	protected function _setJs($arr)
	{
		...
		$vars['portal']['varsNavi']['search']['varsDetail']['varsSearchItem']['templateDetail']['firstOption'] = $this->_removeVarsOption(array(
			'vars' => $vars['portal']['varsNavi']['search']['varsDetail']['varsSearchItem']['templateDetail']['firstOption'],
		));
		$vars['varsItem'] = array();
	 ...
	 *
	 */
	protected function _removeVarsOption($arr)
	{
		$arrayNew = array();
		$array = $arr['vars'];
		foreach ($array as $key => $value) {
			if ($value['strTitle'] == 'dummy') {
				continue;
			}
			$arrayNew[] = $value;
		}
