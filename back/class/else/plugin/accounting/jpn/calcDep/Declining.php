 ...
 * http://rucaro.org/
 */
class Code_Else_Plugin_Accounting_Jpn_CalcDep_Declining extends Code_Else_Plugin_Accounting_Jpn_CalcDep
{
	protected $_varsChildStatic = array(
		'depDecliningNew250' => 'back/tpl/templates/else/plugin/accounting/dat/depDecliningNew250.csv',
		'depDecliningNew200' => 'back/tpl/templates/else/plugin/accounting/dat/depDecliningNew200.csv',
		'depDecliningOld' => 'back/tpl/templates/else/plugin/accounting/dat/depDecliningOld.csv',
	);
	...
	protected function _getVarsNumValueDepCalcBase($arr)
	{
		...
		if ($arr['varsItem']['flag20070331']) {
			...
			$numValueDepCalcBase = $arr['arrValue']['numValueNetOpening']
								+ $arr['arrValue']['numValueDepPrevOver']
								- $arr['arrValue']['numValueDepSpecialShortPrev'];
	...
	{
		$arr['varsItem']['varsDepRate20120401'] = $this->_getVarsCsvKey(array(
			'strKey' => 'numUsefulLife',
			'path'   => $this->_varsChildStatic['depDecliningNew200'],
		));

		$arr['varsItem']['varsDepRate20070401'] = $this->_getVarsCsvKey(array(
			'strKey' => 'numUsefulLife',
			'path'   => $this->_varsChildStatic['depDecliningNew250'],
		));

		$arr['varsItem']['varsDepRate20070331'] = $this->_getVarsCsvKey(array(
			'strKey' => 'numUsefulLife',
			'path'   => $this->_varsChildStatic['depDecliningOld'],
		));
	...
	protected function _setVarsCalc20070401DataUnder($arr)
	{
		...
		$varsDepRate = $arr['varsItem']['varsDepRate20070401'][$numUsefulLife];
		if ($arr['varsItem']['flag20120401']) {
			$varsDepRate = $arr['varsItem']['varsDepRate20120401'][$numUsefulLife];
		}
		$numRateDep = $varsDepRate['numRateDep'];

		$numValueAssured = $this->_updateCalc(array(
			'flagType' => $arr['varsItem']['varsFixedAssets']['flagFractionDep'],
			'num'      => $arr['arrValue']['numValueNet'] * $varsDepRate['numRateDepAssured'],
			'numLevel' => 0
		));
		$this->_varsCalc['numValueAssured'] = $numValueAssured;

		$numValueDepCalcPre = $this->_updateCalc(array(
			'flagType' => $arr['varsItem']['varsFixedAssets']['flagFractionDep'],
			'num'      => $numValueDepCalcBase * $numRateDep,
			'numLevel' => 0
		));

		if ($numValueAssured <= $numValueDepCalcPre) {
			...
			$numRateDepUpdate = $varsDepRate['numRateDepUpdate'];
			$numRateDepUpdate = $this->_updateCalc(array(
				'flagType' => 'ceil',
				'num'      => ($numRateDepUpdate * $numFiscalTermMonth) / 12,
				'numLevel' => 3
			));
		}
	...
	protected function _setVarsCalc20070331DataUnder($arr)
	{
		...
		$numUsefulLife = $arr['arrValue']['numUsefulLife'];
		$numUsefulLifeUpdate = $this->_updateCalc(array(
			'flagType' => 'floor',
			'num'      => ($numUsefulLife * 12) / $numFiscalTermMonth,
			'numLevel' => 0
		));

		$numRateDep = $arr['varsItem']['varsDepRate20070331'][$numUsefulLife]['numRateDep'];
		$numRateDepUpdate = $arr['varsItem']['varsDepRate20070331'][$numUsefulLifeUpdate]['numRateDep'];
		...
		if ($numValueNetOpeningTax > $numSurvivalRateLimit) {
			if ($numUsefulLifeUpdate > 100) {
				$numValueDepCalc = $this->_updateCalc(array(
					'flagType' => $arr['varsItem']['varsFixedAssets']['flagFractionDep'],
					'num'      => $numValueDepCalcBase * $numRateDep * $numDepMonth / 12,
					'numLevel' => 3
				));
				$this->_varsCalc['numRateDep'] = $numRateDep;
				$this->_varsCalc['flagDepRateType'] = 1;
