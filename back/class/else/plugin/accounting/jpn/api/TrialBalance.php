 ...
 * http://rucaro.org/
 */
class Code_Else_Plugin_Accounting_API_Jpn_TrialBalance extends Code_Else_Plugin_Accounting_Jpn_API
{
	 ...
	 */
	protected function _iniGetTrialBalance()
	{
	...
	protected function _updateVars($arr)
	{
		...
		$varsFS = $arr['varsItem']['varsFS']['jsonJgaapAccountTitle' . $arr['varsItem']['varsFlag']['flagFS']];
		...
		$varsData = $this->_getAccountTitleValue(array(
			'vars'        => $arr['vars'],
			'varsFS'      => $varsFS,
			'varsFSValue' => $arr['varsItem']['varsFSValue']['jsonJgaapAccountTitle' . $arr['varsItem']['varsFlag']['flagFS']],
			'varsItem'    => $arr['varsItem'],
	...
	protected function _getAccountTitleValue($arr)
	{
		...
		$arrayValue = array('sumPrev', 'sumDebit', 'sumCredit', 'sumNext');
			...
			if (!is_null($array[$key]['vars']['varsValue'])) {
				...
				foreach ($arrayData as $keyData => $valueData) {
					...
					foreach ($arrayValue as $keyValue => $valueValue) {
						...
						if ($flag) {
							$value['vars']['varsValue'][$flagFiscalPeriod][$valueValue] = $numValue;
						} else {
							$value['vars']['varsValue'][$flagFiscalPeriod][$valueValue] = '';
						}
					}
				}
			}
