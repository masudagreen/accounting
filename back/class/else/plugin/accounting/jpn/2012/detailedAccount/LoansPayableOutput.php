 ...
 * http://rucaro.org/
 */
class Code_Else_Plugin_Accounting_Jpn_2012_DetailedAccount_LoansPayableOutput extends Code_Else_Plugin_Accounting_Jpn_2012_DetailedAccount_LoansPayable
{
	protected $_childSelf = array(
		'pathVarsPrint' => 'back/tpl/vars/else/plugin/accounting/<strLang>/dat/jpn/2012/detailedAccount/loansPayablePrint.php',
	);
	...
	protected function _getVarsLoopPrint($arr)
	{
		...
		$dataValue = $varsValue['valueTextSumPayable'];
		$dataValue = ($dataValue === '' || $dataValue == 0)?  $arr['varsData']['strBlank'] : number_format($dataValue);
		$varsStr['strTextSumPayable'] = $dataValue;
	...
	protected function _getVarsLoopCsv($arr)
	{
		...
		if ($arr['flagFirst']) {
			...
			$rowData[] = $arr['varsData']['strValuePayable'];
		...
		for ($i = 1; $i <= $numEnd; $i++) {
			...
			$dataValue = $varsValue['valueTextValuePayable' . $i];
			$dataValue = ($dataValue === '' || $dataValue == 0)?  '' : $dataValue;
			$strTextValuePayable = $dataValue;
		...
		$dataValue = $varsValue['valueTextSumPayable'];
		$dataValue = ($dataValue === '' || $dataValue == 0)?  '' : $dataValue;
		$strTextSumPayable = $dataValue;

		$arrayCsv[] = array($arr['varsData']['strSum'], '', '', $strTextSum, $strTextSumPayable, '', '', '');
