 ...
 * http://rucaro.org/
 */
class Code_Else_Plugin_Accounting_Jpn_2012_DetailedAccount_DepositsEditor extends Code_Else_Plugin_Accounting_Jpn_2012_DetailedAccount_Deposits
{
	protected $_childSelf = array(
		'pathTplJs'   => 'else/plugin/accounting/js/jpn/2012/detailedAccount/depositsEditor.js',
		'pathVarsJs'  => 'back/tpl/vars/else/plugin/accounting/<strLang>/js/jpn/2012/detailedAccount/depositsEditor.php',
	);
	...
	protected function _getVarsValueDetail($arr)
	{
		...
		for ($i = 1; $i <= $numEnd; $i++) {
			...
			if ($idSubAccountTitle != 0) {
				$varsValue['valueTextBank' . $i] = mb_substr($strSubAccountTitle, 0, $varsTmpl['TextBank']['numMaxlength']);
			}
			$varsValue['valueTextType' . $i] = mb_substr($strAccountTitle, 0, $varsTmpl['TextType']['numMaxlength']);
