 ...
 * http://rucaro.org/
 */
class Code_Else_Plugin_Accounting_Jpn_2012_DetailedAccount_LoansPayableEditor extends Code_Else_Plugin_Accounting_Jpn_2012_DetailedAccount_LoansPayable
{
	protected $_childSelf = array(
		'pathTplJs'   => 'else/plugin/accounting/js/jpn/2012/detailedAccount/loansPayableEditor.js',
		'pathVarsJs'  => 'back/tpl/vars/else/plugin/accounting/<strLang>/js/jpn/2012/detailedAccount/loansPayableEditor.php',
	);
	...
	protected function _getVarsValueDetail($arr)
	{
		...
		for ($i = 1; $i <= $numEnd; $i++) {
			...
			if ($idSubAccountTitle != 0) {
				$varsValue['valueTextBorrower' . $i] = mb_substr($strSubAccountTitle, 0, $varsTmpl['TextBorrower']['numMaxlength']);
			}
	...
	protected function _updateDbPageSum($arr)
	{
		...
		foreach ($array as $key => $value) {
			...
			for ($i = 1; $i <= $numEnd; $i++) {
				$idTarget = 'valueTextValuePayable' . $i;
			...
			if ($value['numPage'] == $numPageMax) {
				$value['jsonData'][$flagMenu]['valueTextSumPayable'] = $sum;

			} else {
				$value['jsonData'][$flagMenu]['valueTextSumPayable'] = '';
			}
