 ...
 * http://rucaro.org/
 */
class Code_Else_Plugin_Accounting_Jpn_2012_DetailedAccount_AccountsReceivableEditor extends Code_Else_Plugin_Accounting_Jpn_2012_DetailedAccount_AccountsReceivable
{
	protected $_childSelf = array(
		'pathTplJs'   => 'else/plugin/accounting/js/jpn/2012/detailedAccount/accountsReceivableEditor.js',
		'pathVarsJs'  => 'back/tpl/vars/else/plugin/accounting/<strLang>/js/jpn/2012/detailedAccount/accountsReceivableEditor.php',
	);
	...
	protected function _getVarsValueDetail($arr)
	{
		...
		for ($i = 1; $i <= $numEnd; $i++) {
			...
			if ($idSubAccountTitle != 0) {
				$varsValue['valueTextName' . $i] = mb_substr($strSubAccountTitle, 0, $varsTmpl['TextName']['numMaxlength']);
			}
			$varsValue['valueTextAccountTitle' . $i] = mb_substr($strAccountTitle, 0, $varsTmpl['TextAccountTitle']['numMaxlength']);
