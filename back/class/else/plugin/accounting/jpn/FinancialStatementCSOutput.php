 ...
 * http://rucaro.org/
 */
class Code_Else_Plugin_Accounting_Jpn_FinancialStatementCSOutput extends Code_Else_Plugin_Accounting_Jpn_FinancialStatementCS
{
	protected $_childSelf = array(
		'pathVarsPrint' => 'back/tpl/vars/else/plugin/accounting/<strLang>/dat/jpn/printFinancialStatementCS.php',
	);
	...
	protected function _getVarsPrint($arr)
	{
		...
		if ($arr['varsFlag']['flagType'] == 'item') {
			...
			$flagCount = 0;
			$array = $arr['varsItem']['varsFlagDirect']['arrStrTitle'];
			foreach ($array as $key => $value) {
				$flagDirect = $key;
				$arr['varsFlag']['flagDirect'] = $flagDirect;
	...
	protected function _getVarsStatus($arr)
	{
		...
		$varsData['strTitleSub'] = $strEntity . '(' . $varsData['strNum'] . ') ' . $strFlagDirect;
	...
	protected function _getVarsLoopPrint($arr)
	{
		...
		foreach ($array as $key => $value) {
			...
			$tmplRow['id'] = $arr['varsFlag']['flagDirect'] . $value['id'];
			$strFirst = '';
			if ($arr['flagFirst']) {
				$arr['flagFirst'] = 0;
