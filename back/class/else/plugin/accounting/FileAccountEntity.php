 ...
 * http://rucaro.org/
 */
class Code_Else_Plugin_Accounting_FileAccountEntity extends Code_Else_Plugin_Accounting_Accounting
{
	protected $_extSelf = array(
		'pathTplJs' => 'else/plugin/accounting/js/fileAccountEntity.js',
		'pathVarsJs'  => 'back/tpl/vars/else/plugin/accounting/<strLang>/js/fileAccountEntity.php',
	);
	...
	protected function _updateSearch($arr)
	{
		...
		$idEntity  = $varsPluginAccountingAccount['idEntityCurrent'];
		$strCheckStamp = 'accountingFileAccountEntity_' . $idEntity . '_' . $numFiscalPeriod;
		...
		foreach ($array as $key => $value) {
			...
			$varsTmpl['strMail'] = $value['strMailFile'];
			$varsTmpl['vars']['strMail'] = $value['strMailFile'];
			$varsTmpl['varsColumnDetail']['strMail'] = $value['strMailFile'];
			if (!$varsTmpl['strMail']) {
				$varsTmpl['varsColumnDetail']['strMail'] = $vars['varsItem']['strNone'];
				$varsTmpl['strClassFont'] = $vars['varsItem']['strClassNone'];
				$varsTmpl['vars']['strMail'] = '';
			}
	...
	protected function _iniNaviSearchSave()
	{
		$this->_setNaviSearchSave(array(
			...
			'strColumn'   => 'jsonFileAccountEntityNaviSearch',
			'flagEntity'  => 1,
			'flagAccount' => 1,
		));
	}
	...
	protected function _iniNaviFolderSave()
	{
		$this->_setNaviFolderSave(array(
			'pathVars'  => $this->_extSelf['pathVarsJs'],
			'strTable'  => 'accountingAccountMemo',
			'strColumn' => 'jsonFileAccountEntityNaviFolder',
			'flagEntity'  => 1,
			'flagAccount' => 1,
		));
	}
