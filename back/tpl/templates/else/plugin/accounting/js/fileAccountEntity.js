 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_FileAccountEntity = Class.create(Code_Lib_ExtPortal,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
	...
	*/
	bootAutoSearch : function(obj)
	{
		...
		if (str == 'arrSpaceStrTag') {
			...
			varsTmpl.flagType = 'str';
			varsTmpl.strColumn = 'accountingAccountEntity.strMailFile';
			varsTmpl.value = obj.vars.vars[str];
			varsData.push(varsTmpl);
		}
	...
	*/
	_getDetailChildVars : function(obj)
	{
		...
		for (var i = 0; i < obj.arr.length; i++) {
			...
			} else if (obj.arr[i].id == 'StrMail') {
				if (obj.flag == 'add') obj.arr[i].value = '';
				else obj.arr[i].value = obj.vars.strMail;
				arrayNew.push(obj.arr[i]);
	...
	_updateDetailListVarsChild : function(obj)
	{
		...
		for (var i = 0; i < obj.arr.length; i++) {
			...
			if (obj.arr[i].id == 'StampRegister') {
				...
				obj.arr[i].value = obj.vars.varsColumnDetail.strMail;
				if (!obj.vars.vars[id]) {
					obj.arr[i].varsTextBtn = null;
					arrayNew.push(obj.arr[i]);
					continue;
