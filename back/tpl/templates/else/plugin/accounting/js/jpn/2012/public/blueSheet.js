 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_BlueSheet = Class.create(Code_Lib_ExtPortal,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
	...
	*/
	_eventDetailConnect : function(obj)
	{
		if (obj.flag == 'reload') {
			...
			var arr = this.vars.varsItem.varsList;
			for (var i = 0; i < arr.length; i++) {
				if (arr[i].idTarget == 'valueNozeisha_id') {
					if (arr[i].value == '') {
						alert(this.vars.varsComment.strCaution);
						return;
					}

				} else if (arr[i].idTarget == 'valueSelectTypeZeimusho') {
					if (arr[i].value == '') {
						alert(this.vars.varsComment.strCaution);
						return;
					}
