 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_DetailedAccountLoansReceivableEditor = Class.create(Code_Lib_ExtEditor,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
{literal}
	...
	*/
	_updateSensitiveStep : function(obj)
	{
		...
		this._updateVarsTextSumReceived({arr : obj.arr});
	},
	...
	*/
	 ...
	 *
	*/
	_updateVarsTextSumReceived : function(obj)
	{
		var flagMenu = this._varsFlag.flagMenu;
		if (!flagMenu.match(/^detail/)) {
			return;
		}
		var idTarget = 'TextSumReceived';
		var numPage = this._varsFlag.numPage;
		var numPageMax = this._varsPreference.jsonData.numPageMax;
		if (numPage != numPageMax) {
			this._varsSensitive.ins.updateVarsTarget({
		...
		var varsTextSumReceived = this._getVarsDetailIniSensitive({idTarget : idTarget});
		var numSum = varsTextSumReceived.value;
