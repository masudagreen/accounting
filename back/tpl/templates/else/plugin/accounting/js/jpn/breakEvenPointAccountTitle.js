 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_BreakEvenPointAccountTitle = Class.create(Code_Lib_ExtPortal,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
		...
		for (var i = 0; i < obj.arr.length; i++) {
			if (obj.arr[i].id == 'DummyComment') {
				...
				var strDepartment = this._getDetailChildVarsStrDepartment({
					arr      : this.insNavi.insForm.vars.varsDetail,
					idTarget : obj.vars.varsFlag.idDepartment,
					str      : obj.arr[i].varsTmpl.strDepartment
				});
				var strAccountTitle = obj.vars.strTitle;
				var str = strAccountTitle + ' , ' + strDepartment;
				if (flagDepartment) {
					str = strAccountTitle;
				}
				...
				obj.arr[i].value = obj.vars.varsFlag.idDepartment;
				arrayNew.push(obj.arr[i]);
			...
			} else if (obj.arr[i].id == 'FlagType') {
				obj.arr[i].value = obj.vars.varsType.flagType;
				arrayNew.push(obj.arr[i]);
			}
		}
	...
	*/
	_updateDetailListVarsChild : function(obj)
	{
		...
		for (var i = 0; i < obj.arr.length; i++) {
			...
			} else if (obj.arr[i].id == 'DummyFlagType') {
				obj.arr[i].value = obj.vars.varsColumnDetail.flagType;
				arrayNew.push(obj.arr[i]);
			}
		}
