 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_SummaryStatementPublicEditor = Class.create(Code_Lib_ExtEditor,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
{literal}
	...
	*/
	...
	_setVarsData : function(obj)
	{
		...
		this._varsValue = (Object.toJSON(this.insCurrent.vars.varsItem.varsSave)).evalJSON();
	...
	*/
	_updateVarsDetailSensitive : function(obj)
	{
		...
		} else if (obj.vars.flagValueType.match(/^(rate)/)) {
			if (obj.vars.value != '') {
				var flag = 0;
				if (obj.vars.flagValueType == 'rate') {
					if (!obj.vars.value.match(/^[0-9]{1,3}\.[0-9]{2,2}$/)) {
						flag = 1;
					}
				} else if (obj.vars.flagValueType == 'rateOne') {
					if (!obj.vars.value.match(/^[0-9]{1,3}\.[0-9]{1,1}$/)) {
						flag = 1;
					}
				}
				if (flag) {
