 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_FinancialStatementMultiCS = Class.create(Code_Lib_ExtPortal,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
{literal}
	...
	_setListFormChartBar : function(obj)
	{
		...
		for (var j = 0; j < arrPeriodNum.length; j++) {
			...
			for (var i = 0; i < arr.length; i++) {
				...
				arrPeriodStrTitle.push(strTitle);
				if (flagFiscalPeriod == 'f1') {
					if (str.match(/^f1$/)) {
						tempData.push(num);
		...
		var tempData = {
			data  : [],
			label : varsCollect.arrStrTitle[idAccountTitle].strTitle
		};
		...
		for (var j = 0; j < arrPeriod.length; j++) {
			...
			var numValue = varsCollect.varsBase[numFiscalPeriod].varsNum[idPeriod][idAccountTitle].sumNext;
			if (numValue !== '') {
	...
	*/
	...
	_setDetailFormChartBar : function(obj)
	{
		...
		var varsIdAccountTitle = vars.IdAccountTitle;
		...
		var numData = 0;
		var numMaxValue = 0;
		var arrIdAccountTitle = varsCollect.arrSelectTag;
		for (var i = 0; i < arrIdAccountTitle.length; i++) {
			if (!varsIdAccountTitle[arrIdAccountTitle[i].value]) {
				continue;
			}
			numMax++;
			if (numMax > 10) {
				varsIdAccountTitle[arrIdAccountTitle[i].value] = 0;
				continue;
			}

			var idAccountTitle = arrIdAccountTitle[i].value;
			var tempData = {
				data  : [],
				label : varsCollect.arrStrTitle[idAccountTitle].strTitle
			};
		...
		if (numMax > 10) {
			this.insDetail.setFormValue({
				flag     : 'selectMultiple',
				idTarget : 'IdAccountTitle',
				value    : varsIdAccountTitle
			});
		}
