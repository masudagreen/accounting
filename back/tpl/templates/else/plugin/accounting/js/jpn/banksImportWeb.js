 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_BanksImportWeb = Class.create(Code_Lib_ExtPortal,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
	...
	_eventDetailConnectSuccess : function(obj)
	{
		if (obj.json.flag == 1) {
			if (this._varsDetailConnect.flag == 'save') {
				this._setDetailEnd({
					strHtml   : obj.json.data.strComment,
					flagRetry : parseFloat(obj.json.data.flagRetry),
					flagLog   : parseFloat(obj.json.data.flagLog),
				});
				if (parseFloat(obj.json.data.flagImport)) {
			...
			var str = '';
			if (obj.json.data) {
				if (obj.json.data.strComment) {
					str = (obj.json.data.strComment)? obj.json.data.strComment : '';
				}
			}
