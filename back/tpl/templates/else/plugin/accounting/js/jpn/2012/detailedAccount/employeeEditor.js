 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_DetailedAccountEmployeeEditor = Class.create(Code_Lib_ExtEditor,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
{literal}
	...
	*/
	_updateVarsTextSum : function(obj)
	{
		...
		var arrStr = ['RewardSum', 'Employee', 'EmployeeRegular', 'EmployeePrev', 'EmployeeProfit', 'Others' ,''];
		for (var j = 0; j < arrStr.length; j++) {
			var idTarget = 'TextSum' + arrStr[j];
		...
		for (var j = 0; j < arrStr.length; j++) {
			...
			var numEnd = this._varsCommon.varsStr[flagMenu].numRowsLabor;
			for (var i = 1; i <= numEnd; i++) {
				var id = 'TextValue' + arrStr[j] + i;
				var data = varsValue[id];
				if (data == '' || data == 0) {
