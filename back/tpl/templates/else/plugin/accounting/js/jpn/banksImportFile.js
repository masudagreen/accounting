 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_BanksImportFile = Class.create(Code_Lib_ExtPortal,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
{literal}
	...
	*/
	...
	_checkAutoSearch : function(obj)
	{
		...
		if (this._flagAutoData == 'Log' || this._flagAutoData == 'LogImportRetry') {
			var idTarget = 'Log';
	...
	eventAutoSearch : function()
	{
		...
		} else if (this._flagAutoData == 'Log') {
	...
	_getDetailAllot : function()
	{
		var allot = function(obj)
		{
			...
			if (obj.from == 'form-preEventLayout') insCurrent._preEventLayoutDetailContent();
				...
				} else if (obj.vars.vars.vars.flagLog) {
					insCurrent._checkAutoSearch({idTarget : 'Log'});
					insCurrent.insDetail.showBtnBottom();
	...
	*/
	_eventDetailConnect : function(obj)
	{
		if (obj.flag.match(/^reload$/)) {
			...
			var varsBanksAccount = this.vars.varsItem.varsBanksAccountList.arrStrTitle[vars.IdLogAccount];
			var varsBanks = this.vars.varsItem.varsBanksList[varsBanksAccount.flagBank];
			...
			if (this._varsDetailConnect.flag == 'save') {
				...
				for (var i = 0; i < arr.length; i++) {
					...
					if (arr[i].flagConvertError) {
						tmplstr = this.vars.varsItem.varsOption.varsComment.strConvertError;

					} else if (arr[i].flagLog) {
						tmplstr = this.vars.varsItem.varsOption.varsComment.strStatusLog;
						...
						tmplstr = this.vars.varsItem.varsOption.varsComment.strStatusBanks;
						if (arr[i].replaceAllImport > 0) {
							flagImport = 1;
						}
					}
