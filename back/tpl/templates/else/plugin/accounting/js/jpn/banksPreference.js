 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_BanksPreference = Class.create(Code_Lib_ExtPreference,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
{literal}
	...
	*/
	...
	bootAutoSearchOver : function(obj)
	{
		...
		} else if (obj.flag == 'addBanksAccount') {
			obj.flag.match(/^add(.*?)$/);
			var idTarget = RegExp.$1;
			var varsData = this.checkChildData({idTarget : idTarget});
			if (!varsData) {
	...
	{
		if (this._flagAutoSearchOver == 'addBanksAccount') {
