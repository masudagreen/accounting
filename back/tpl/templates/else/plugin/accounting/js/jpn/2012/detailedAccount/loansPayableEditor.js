 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_DetailedAccountLoansPayableEditor = Class.create(Code_Lib_ExtEditor,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
{literal}
	...
	*/
	_updateSensitiveStep : function(obj)
	{
		...
		this._updateVarsTextSumPayable({arr : obj.arr});
	},
	...
	*/
	 ...
	 *
	*/
	_updateVarsTextSumPayable : function(obj)
	{
		var flagMenu = this._varsFlag.flagMenu;
		if (!flagMenu.match(/^detail/)) {
			return;
		}
		var idTarget = 'TextSumPayable';
		var numPage = this._varsFlag.numPage;
		var numPageMax = this._varsPreference.jsonData.numPageMax;
		if (numPage != numPageMax) {
			this._varsSensitive.ins.updateVarsTarget({
		...
		var varsTextSumPayable = this._getVarsDetailIniSensitive({idTarget : idTarget});
		var numSum = varsTextSumPayable.value;
