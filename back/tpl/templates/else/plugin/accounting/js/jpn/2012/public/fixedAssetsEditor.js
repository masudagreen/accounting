...
{literal}
	...
	*/
	 ...
	 * unique
	*/
	_updateVarsNumRatioOperate : function(obj)
	{
		if (obj.strError) {
			this._varsSensitive.ins.updateVarsTarget({
				idTarget : obj.vars.id,
		...
		if (obj.vars.value == '') {
			...
			if (obj.vars.value > 100) {
				this._setViewError({
					idTarget   : obj.vars.idError,
					strComment : obj.vars.varsError.strMax
				});
				this._varsSensitive.ins.updateVarsTarget({
					idTarget : obj.vars.id,
					strKey   : 'value',
		...
		this._updateVarsCalcDepNumValueDepOperate({
			varsValue     : varsValue,
	...
	*/
	 ...
	 * unique
	*/
	_updateVarsCalcDepNumValueDepOperate : function(obj)
	{
		var numValueDep = this._getNumValueTarget({idTarget : 'NumValueDep'});
		var numRatioOperate = this._getNumValueTarget({idTarget : 'NumRatioOperate'});

		var num = this._getNumValueMathCalc({
			flagCalc : 'flagFractionRatioOperate',
			num      : numValueDep * numRatioOperate / 100
		});

		if (!(obj.flagDepMethod == 'sum' || obj.flagDepMethod == 'noneDep')) {
			this._varsSensitive.ins.updateVarsTarget({
				idTarget : 'NumValueDepOperate',
				strKey   : 'value',
				vars     : num
			});
		}
