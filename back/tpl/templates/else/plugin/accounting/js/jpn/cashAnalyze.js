 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_CashAnalyze = Class.create(Code_Lib_ExtPortal,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
{literal}
	...
	_setDetailFormChartBar : function(obj)
	{
		...
		for (var i = 0; i < arr.length; i++) {
			...
			for (var j = 0; j < arrPeriod.length; j++) {
				...
				varsTable[arr[i]][flagFiscalPeriod] = data;
				tempData.data.push([num, numValue]);
				num++;
			}
			varsData.push(tempData);
	...
	*/
	...
	{
		if (!obj.varsCollect.varsCashValue[obj.flagFiscalPeriod]) {
			return 0;
		}
		if (!obj.varsCollect.varsCashValue[obj.flagFiscalPeriod].varsContra) {
			return 0;
		}
		if (!obj.varsCollect.varsCashValue[obj.flagFiscalPeriod].varsContra[obj.idAccountTitle]) {
			return 0;
		}
		if (!obj.varsCollect.varsCashValue[obj.flagFiscalPeriod].varsContra[obj.idAccountTitle][obj.strDetail]) {
			return 0;
		}
		var numValue = parseFloat(obj.varsCollect.varsCashValue[obj.flagFiscalPeriod].varsContra[obj.idAccountTitle][obj.strDetail][obj.flagIn]);
	...
	*/
	_setDetailFormTableDetail : function(obj)
	{
		...
		strColumn += tagTdColumn.interpolate({insertPoint : '', numWidth : numWidth});

		var arr = obj.varsTable.arrPeriod;
		for (var i = 0; i < arr.length; i++) {
			var tagTdColumn = obj.vars.tagTdColumn;
			var str = varsCollect.varsStrFlagFiscalPeriod[arr[i]];
		...
		for (var i = 0; i < arr.length; i++) {
			...
			for (var j = 0; j < arrPeriod.length; j++) {
				...
				var str = obj.varsTable.varsTable[arr[i]][arrPeriod[j]].numValueComma;
