 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_BudgetEditor = Class.create(Code_Lib_ExtEditor,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
	...
	_setVarsDataLoop : function(obj)
	{
		for (var i = 0; i < obj.arr.length; i++) {
			...
			if (obj.arr[i].vars.varsValue != undefined) {
				...
				if (obj.arr[i].vars.flagCalc == 'sum' || obj.arr[i].vars.flagCalc == 'net') {
					vars.flagForm = 'none';
				}
				vars.id = obj.arr[i].vars.idTarget;
				vars.value = obj.arr[i].varsValue.numBudget;
				if (obj.flagUpdate) {
	...
	_updateVarsTreeValue : function(obj)
	{
		for (var i = 0; i < obj.arr.length; i++) {
			if (obj.arr[i].vars.idTarget == obj.idTarget) {
				obj.arr[i].varsValue.numBudget = parseFloat(obj.value);
				return 1;
		...
		for (var i = 0; i < obj.arr.length; i++) {
			...
			if (obj.arr[i].vars.varsValue != undefined) {
				...
				if (obj.arr[i].vars.flagCalc == 'sum') {
					...
					if (obj.arr[i].varsValue.numBudget != '') {
						numNext = parseFloat(obj.arr[i].varsValue.numBudget);
					}
				}
	...
	*/
	_setVarsTreeValue : function(obj)
	{
		for (var i = 0; i < obj.arr.length; i++) {
			...
			if (num != undefined) {
				obj.arr[i].varsValue.numBudget = parseFloat(num);
	...
	_eventDetailConnectSuccess : function(obj)
	{
		if (obj.json.flag == 1) {
			if (this._varsDetailConnect.flag == 'add') {
				...
				this.insCurrent._eventNaviConnect({flag : 'search'});
				this._setDetailEnd();
			}
