 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_DetailedAccountLandEditor = Class.create(Code_Lib_ExtEditor,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
{literal}
	...
	*/
	_updateSensitiveStep : function(obj)
	{
		...
		this._updateVarsTextSumMargin({arr : obj.arr});
	},
	...
	*/
	 ...
	 *
	*/
	_updateVarsTextSumMargin : function(obj)
	{
		var flagMenu = this._varsFlag.flagMenu;
		if (!flagMenu.match(/^detail/)) {
			return;
		}
		var idTarget = 'TextSumMargin';
		var numPage = this._varsFlag.numPage;
		var numPageMax = this._varsPreference.jsonData.numPageMax;
		if (numPage != numPageMax) {
			this._varsSensitive.ins.updateVarsTarget({
		...
		var varsTextSumMargin = this._getVarsDetailIniSensitive({idTarget : idTarget});
		var numSum = varsTextSumMargin.value;
