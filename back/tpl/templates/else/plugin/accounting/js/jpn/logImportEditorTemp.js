 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_LogImportEditorTemp = Class.create(Code_Lib_ExtEditor,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
{literal}
	...
	*/
	iniLoad : function(obj)
	{
		...
		this._iniNavi();
		this._iniDetail();
		if (this.insTop.vars.flagIdAccountTitle) {
			this._eventDetailConnect({flag : 'estimate'});
		}
	},
	...
	*/
	eventWindowAppear : function(obj)
	{
		...
		if (this.varsChild.varsTreePast) {
			this.insNavi.setVarsTreePast({vars : this.varsChild.varsTreePast});
		}
	...
	*/
	_eventDetailConnectSuccess : function(obj)
	{
		...
		if (obj.json.flag == 1) {
			...
			this.insCurrent.bootAutoSearchOver({flag : 'loopFilter'});
			var varsTreePast = this.insNavi.getVarsTreePast();
			this.insCurrent.bootAutoSearchOver({flag : 'reloadMemo', vars : varsTreePast});
