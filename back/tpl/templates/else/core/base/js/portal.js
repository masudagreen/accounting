	...
	_setNaviDetailStrPassword : function(obj)
	{
		...
		for (var i = 0; i < obj.arr.length; i++) {
			if (obj.arr[i].id == 'DummyPassword') {
				...
				obj.arr[i].strComment = obj.arr[i].strComment.replace(/<%replace%>/, strTime);
			}
		}
		...
		if (obj.flag == 'reload') {
			...
			if (obj.idTarget == 'strPassword') {
				...
				if (strDomainUrl != strDomainMail) {
					...
					strConfirm = strConfirm.replace(/<%strDomainUrl%>/, strDomainUrl);
					strConfirm = strConfirm.replace(/<%strDomainMail%>/, strDomainMail);
