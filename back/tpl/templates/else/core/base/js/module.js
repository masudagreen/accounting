 ...
 * http://rucaro.org/
 */
var Code_Core_Base_Module = Class.create(Code_Lib_ExtPortal,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
	...
	*/
	_getDetailChildVars : function(obj)
	{
		...
		for (var i = 0; i < obj.arr.length; i++) {
			...
			} else if (obj.arr[i].id == 'JsonModule') {

				if (obj.flag == 'add') {
					var arr = (Object.toJSON(obj.arr[i].varsFormCheck.jsonModule)).evalJSON();
					var array = [];
					for (var j = 0; j < arr.length; j++) {

						var varsDetail = (Object.toJSON(obj.arr[i].varsFormCheck.templateDetail)).evalJSON();
						varsDetail.id = arr[j].idModule.capitalize();
						varsDetail.value = 'User';
						varsDetail.idModule = arr[j].idModule;
						varsDetail.varsColumnDetail.strModule = arr[j].strModule;
						varsDetail.varsColumnDetail.flagAdminNow = 0;
						varsDetail.varsColumnDetail.flagUserNow = 1;
						varsDetail.varsColumnDetail.flagAdminLock = 0;
						varsDetail.varsColumnDetail.flagUserLock = 0;


						if (arr[j].idModule == 'base') {
							varsDetail.value = 'User';
							varsDetail.varsColumnDetail.flagUserLock = 1;
							varsDetail.varsColumnDetail.flagUserNow = 1;
						}
						array.push(varsDetail);
					}
					obj.arr[i].varsFormCheck.varsDetail = array;

				} else {
					var arr = obj.vars.jsonModule;
					var array = [];
					var flag = 0;
					for (var j = 0; j < arr.length; j++) {
						if (arr[j].idModule == 'base' && arr[j].value == 'Admin') {
							flag = 1;
							break;
						}
					}
					...
					for (var j = 0; j < arr.length; j++) {
						...
						if (flag) {
							if (arr[j].idModule != 'base') {
								varsDetail.varsColumnDetail.flagAdminNow = 1;
								varsDetail.varsColumnDetail.flagUserNow = 1;
								varsDetail.varsColumnDetail.flagAdminLock = 1;
								varsDetail.varsColumnDetail.flagUserLock = 1;

							} else if (arr[j].value == 'Admin') {
								varsDetail.varsColumnDetail.flagAdminNow = 1;

							} else if (arr[j].value == 'User') {
								varsDetail.varsColumnDetail.flagAdminNow = 0;

							}

						} else {
							if (arr[j].value == 'Admin') {
								varsDetail.varsColumnDetail.flagAdminNow = 1;
								varsDetail.varsColumnDetail.flagUserNow = 1;
								...
								varsDetail.varsColumnDetail.flagAdminNow = 0;
								varsDetail.varsColumnDetail.flagUserNow = 0;
