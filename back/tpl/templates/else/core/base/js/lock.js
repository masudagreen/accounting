 ...
 * http://rucaro.org/
 */
var Code_Core_Base_Lock = Class.create(Code_Lib_ExtPortal,
{
{/literal}
	vars : {$varsLoad},
	numNews : {$numNews},
{literal}
	...
	_setNaviDetailChild : function(obj)
	{
		...
		for (var i = 0; i < obj.arr.length; i++) {
			if (obj.arr[i].id == 'StampRegister') {
				...
				obj.arr[i].strComment = obj.arr[i].strComment.replace(/<%strTitle%>/, obj.vars.vars.strCodeName);
	...
	*/
	_eventDetailConnectSuccess : function(obj)
	{
		...
		} else if (obj.json.flag == 'idLoginPast'
