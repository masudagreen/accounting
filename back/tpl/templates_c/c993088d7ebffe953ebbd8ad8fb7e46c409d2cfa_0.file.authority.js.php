<?php /* Smarty version 3.1.24, created on 2019-10-06 09:30:24
         compiled from "/app/rucaro/back/tpl/templates/else/plugin/accounting/js/authority.js" */ ?>
<?php
/*%%SmartyHeaderCode:16605497815d99b430b77151_47218510%%*/
if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    'c993088d7ebffe953ebbd8ad8fb7e46c409d2cfa' => 
    array (
      0 => '/app/rucaro/back/tpl/templates/else/plugin/accounting/js/authority.js',
      1 => 1570328744,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '16605497815d99b430b77151_47218510',
  'variables' => 
  array (
    'varsLoad' => 0,
    'numNews' => 0,
  ...
  'version' => '3.1.24',
  'unifunc' => 'content_5d99b430c982d7_19711806',
),false);
/*/%%SmartyHeaderCode%%*/
if ($_valid && !is_callable('content_5d99b430c982d7_19711806')) {
function content_5d99b430c982d7_19711806 ($_smarty_tpl) {

$_smarty_tpl->properties['nocache_hash'] = '16605497815d99b430b77151_47218510';
?>
 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_Authority = Class.create(Code_Lib_ExtPortal,
{
...
,
	...
	*/
	_getDetailChildVars : function(obj)
	{
		...
		for (var i = 0; i < obj.arr.length; i++) {
			...
			} else if (obj.arr[i].id == 'JsonAuthority') {
				if (obj.flag == 'add') {


				} else {
					this._updateDetailListVarsChildJsonAuthority({
						flagLock  : 0,
						arrValue  : obj.vars.jsonAuthority,
						arrColumn : obj.arr[i].varsFormCheck.varsColumn,
						arr       : obj.arr[i].varsFormCheck.varsDetail,
					});
				}
	...
	*/
	 ...
	 *
	*/
	_updateDetailListVarsChildJsonAuthority : function(obj)
	{
		var insEscape = new Code_Lib_Escape();
		...
		for (var i = 0; i < obj.arr.length; i++) {
			for (var j = 0; j < obj.arrColumn.length; j++) {
				...
				var strNow = id + 'Now';
				var strLock = id + 'Lock';
				obj.arr[i].varsColumnDetail[strNow] = parseFloat(obj.arrValue[obj.arr[i].id][strNow]);

				if (obj.flagLock) obj.arr[i].varsColumnDetail[strLock] = 1;
			}
			if (obj.arr[i].id == 'All') {
				if (!obj.arr[i].varsColumnDetail.flagSelectNow) {
					for (var j = 0; j < obj.arrColumn.length; j++) {
						if (obj.arrColumn[j].id == 'StrType' || obj.arrColumn[j].id == 'FlagSelect') continue;
						var id = insEscape.strLowCapitalize({data : obj.arrColumn[j].id});
						var strNow = id + 'Now';
						var strLock = id + 'Lock';
					...
					for (var j = 0; j < obj.arrColumn.length; j++) {
						...
						var strNow = id + 'Now';
						var strLock = id + 'Lock';
						if (parseFloat(obj.arrValue.All[strNow]) && parseFloat(obj.arrValue.My[strNow])) {
