<?php /* Smarty version 3.1.24, created on 2018-08-13 06:15:34
         compiled from "/storage/emulated/0/htdocs/rucaro/back/tpl/templates/else/plugin/accounting/js/accountEntityAuthority.js" */ ?>
<?php
/*%%SmartyHeaderCode:5479841185b712206ad5689_85086246%%*/
if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '57cc3ccd6b1f885e2893e0227d4639acea4c80bd' => 
    array (
      0 => '/storage/emulated/0/htdocs/rucaro/back/tpl/templates/else/plugin/accounting/js/accountEntityAuthority.js',
      1 => 1483698230,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '5479841185b712206ad5689_85086246',
  'variables' => 
  array (
    'varsLoad' => 0,
    'numNews' => 0,
  ...
  'version' => '3.1.24',
  'unifunc' => 'content_5b712206cafa26_19057899',
),false);
/*/%%SmartyHeaderCode%%*/
if ($_valid && !is_callable('content_5b712206cafa26_19057899')) {
function content_5b712206cafa26_19057899 ($_smarty_tpl) {

$_smarty_tpl->properties['nocache_hash'] = '5479841185b712206ad5689_85086246';
?>
 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_AccountEntityAuthority = Class.create(Code_Lib_ExtPortal,
{
...
,
	...
	*/
	bootAutoSearch : function(obj)
	{
		...
		} else if (str == 'dummyAccess') {
			varsTmpl.flagType = 'num';
			varsTmpl.strColumn = 'idAccess';
			varsTmpl.value = obj.vars.vars[str];
			varsData.push(varsTmpl);

		} else if (str == 'dummyAuthority') {
			varsTmpl.flagType = 'num';
			varsTmpl.strColumn = 'idAuthority';
			varsTmpl.value = obj.vars.vars[str];
			varsData.push(varsTmpl);
		}
...
/, obj.vars.strTitle);
			...
			} else if (obj.arr[i].id == 'IdAuthority') {
				if (obj.flag == 'add') {
				...
				if (obj.flag == 'add') {
					...
					obj.arr[i].value = obj.vars.idAccess;
					var data = (Object.toJSON(obj.arr[i].varsFormList.templateDetail)).evalJSON();
					data.value = obj.vars.strAccess;
					obj.arr[i].varsFormList.varsDetail.push(data);
					arrayNew.push(obj.arr[i]);
	...
	*/
	_updateDetailListVarsChild : function(obj)
	{
		...
		for (var i = 0; i < obj.arr.length; i++) {
			...
			} else if (obj.arr[i].id == 'DummyAuthority') {
				obj.arr[i].value = obj.vars.strAuthority;
				var temp = {};
				temp.id = obj.arr[i].id;
				temp.strTitle = obj.arr[i].value;
				temp.vars = {};
				temp.vars[id] = obj.vars.vars.idAuthority;
				temp.vars.idTarget = obj.arr[i].id;
				obj.arr[i].varsTextBtn.push(temp);
				arrayNew.push(obj.arr[i]);

			} else if (obj.arr[i].id == 'DummyAccess') {
				obj.arr[i].value = obj.vars.strAccess;
				var temp = {};
				temp.id = obj.arr[i].id;
				temp.strTitle = obj.arr[i].value;
				temp.vars = {};
				temp.vars[id] = obj.vars.vars.idAccess;
				temp.vars.idTarget = obj.arr[i].id;
				obj.arr[i].varsTextBtn.push(temp);
				arrayNew.push(obj.arr[i]);
