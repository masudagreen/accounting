<?php /* Smarty version 3.1.24, created on 2019-06-28 03:21:15
         compiled from "/storage/emulated/0/htdocs/rucaro/back/tpl/templates/else/plugin/accounting/js/jpn/logEditor.js" */ ?>
<?php
/*%%SmartyHeaderCode:11985746925d1587abac4568_53596447%%*/
if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    'e15c93d2bd4127da54d42c572132b2c7bdc3a2db' => 
    array (
      0 => '/storage/emulated/0/htdocs/rucaro/back/tpl/templates/else/plugin/accounting/js/jpn/logEditor.js',
      1 => 1560675145,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '11985746925d1587abac4568_53596447',
  'variables' => 
  array (
    'varsLoad' => 0,
    'numNews' => 0,
  ...
  'version' => '3.1.24',
  'unifunc' => 'content_5d1587abdec238_60939966',
),false);
/*/%%SmartyHeaderCode%%*/
if ($_valid && !is_callable('content_5d1587abdec238_60939966')) {
function content_5d1587abdec238_60939966 ($_smarty_tpl) {

$_smarty_tpl->properties['nocache_hash'] = '11985746925d1587abac4568_53596447';
?>
 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_LogEditor = Class.create(Code_Lib_ExtEditor,
{
...
,
	...
	*/
	_setDetailDefer : function()
	{
		...
		var objData = {
			...
			strClass : null,
			vars     : {
				varsDetail : this.vars.portal.varsDetail.varsDefer.varsDetail,
				varsBtn    : this.vars.portal.varsDetail.varsDefer.varsBtn,
				varsEdit   : {},
				vars       : {}
			}
		};
	 ...
	 *
	*/
	_setDetailDeferReject : function()
	{
		this._varsDetailEnd = (Object.toJSON(this.insDetail.varsEventList)).evalJSON();
		this._getDetailFormContentVars({arr : this.insDetail.insForm.vars.varsDetail});
		this._varsDetailEnd.varsDetail = (Object.toJSON(this.insDetail.insForm.vars.varsDetail)).evalJSON();
		var objData = {
			...
			strClass : null,
			vars     : {
				varsDetail : this.vars.portal.varsDetail.varsDeferReject.varsDetail,
				varsBtn    : this.vars.portal.varsDetail.varsDeferReject.varsBtn,
				varsEdit   : {},
				vars       : {}
			}
		};
	...
	*/
	...
	{
		this._setDetailFormView({arr : this.insDetail.insForm.vars.varsDetail});
	...
	*/
	 ...
	 *
	*/
	_setDetailFormView : function(obj)
	{
		for (var i = 0; i < obj.arr.length; i++) {
			if (obj.arr[i].id == 'StampBook') {
				if ($(this.insDetail.insForm.idSelf + 'FlagFiscalReport').value == 'none') {
	...
	*/
	...
	{
		var varsData = this.insTop.checkChildData({idTarget : this._flagAutoData});
		varsData.insClass.bootAutoSearchOver({flag : 'addFile'});
	},
	...
	_updateDetailFormAreaVars : function(obj)
	{
		for (var i = 0; i < obj.arr.length; i++) {
			...
			if (obj.varsIdAccount.idAccountApply == obj.arr[i].vars.idTarget) {
				...
				obj.arr[i].strTitle += obj.varsTmpl.strApply;
			}
		...
		for (var i = 0; i < obj.arr.length; i++) {
			...
			if (obj.idTarget == 'ArrCommaIdAccountPermit') {
				if (obj.varsIdAccount.idAccountApply == obj.arr[i].vars.idTarget) {
					...
					varsTmpl.strTitle += obj.insArea.vars.varsTmpl.strApply;
				}
	...
	*/
	_eventDetailConnect : function(obj)
	{
		...
		if (obj.flag == 'reload') {
			...
			} else if (vars.JsonDetail.varsDetail.length > 10) {
			...
			} else if (vars.FlagFiscalReport == 'f1') {
				stamp = cut.varsStampTerm.stampMax;

			} else if (vars.FlagFiscalReport == 'f21') {
				stamp = cut.varsStampTerm2.stampMax;
			}
			...
			for (var i = 0; i < obj.arr.length; i++) {
				if (!obj.vars[obj.arr[i].id]) continue;
				if (obj.arr[i].id == 'ArrCommaIdAccountPermit') {
	...
	*/
	_eventDetailConnectSuccess : function(obj)
	{
		if (obj.json.flag == 1) {
			...
			this._setDetailDefer();

		} else if (obj.json.flag == 'deferReject') {
			this._setDetailDeferReject();
	...
	*/
	_checkDetailJournalEstimate : function(obj)
	{
		...
		for (var i = 0; i < obj.arr.length; i++) {
			...
			if (!cut.arrAccountTitle.arrStrTitle[obj.arr[i].idAccountTitleDebit]
				|| !cut.arrAccountTitle.arrStrTitle[obj.arr[i].idAccountTitleCredit]
			) {
				continue;
			}
