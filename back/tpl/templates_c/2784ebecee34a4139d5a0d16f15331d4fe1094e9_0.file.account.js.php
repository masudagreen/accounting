<?php /* Smarty version 3.1.24, created on 2022-08-13 00:24:16
         compiled from "/var/www/html/accounting/back/tpl/templates/else/plugin/accounting/js/account.js" */ ?>
<?php
/*%%SmartyHeaderCode:61026556662f6ef304dc5f1_03308302%%*/
if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '2784ebecee34a4139d5a0d16f15331d4fe1094e9' => 
    array (
      0 => '/var/www/html/accounting/back/tpl/templates/else/plugin/accounting/js/account.js',
      1 => 1395308462,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '61026556662f6ef304dc5f1_03308302',
  'variables' => 
  array (
    'varsLoad' => 0,
    'numNews' => 0,
  ...
  'version' => '3.1.24',
  'unifunc' => 'content_62f6ef3054a867_47003121',
),false);
/*/%%SmartyHeaderCode%%*/
if ($_valid && !is_callable('content_62f6ef3054a867_47003121')) {
function content_62f6ef3054a867_47003121 ($_smarty_tpl) {

$_smarty_tpl->properties['nocache_hash'] = '61026556662f6ef304dc5f1_03308302';
?>
 ...
 * http://rucaro.org/
 */
var Code_Plugin_Accounting_Account = Class.create(Code_Lib_ExtPortal,
{
...
,
	...
	*/
	bootAutoSearch : function(obj)
	{
		...
		if (str == 'arrSpaceStrTag') {
			...
			varsTmpl.strColumn = 'baseAccount.arrSpaceStrTag';
			varsTmpl.value = ' ' + obj.vars.vars.strTag + ' ';
			varsData.push(varsTmpl);

		} else if (str == 'flagAdmin') {
			...
			varsTmpl.flagType = 'num';
			varsTmpl.strColumn = 'baseAccount.id';
			varsTmpl.value = obj.vars.vars[str];
			varsData.push(varsTmpl);

		} else if (str == 'strTitle') {
			varsTmpl.flagType = 'str';
			varsTmpl.strColumn = 'baseAccount.strCodeName';
			varsTmpl.value = obj.vars.vars[str];
			varsData.push(varsTmpl);
		}
	...
	*/
	...
	_iniDetailFormArea : function()
	{
		...
		this._setDetailFormAreaVars({arr : this.insDetail.insView.vars.varsDetail});
		this._setDetailFormArea({arr : this.insDetail.insView.vars.varsDetail});
	},
	...
	*/
	...
	{
		if (!this.insDetail.varsEventList) return;
		this._eventRemoveDetailFormArea({arr : this.insDetail.insView.vars.varsDetail});
	},
...
/, obj.vars.strTitle);
			...
			} else if (obj.arr[i].id == 'ArrCommaIdEntity') {
				if (obj.vars.flagAdmin) continue;
				var arr = obj.vars.varsEntity;
			...
			} else if (obj.arr[i].id == 'DummyAdmin') {
				if (obj.vars.flagAdmin) arrayNew.push(obj.arr[i]);
	...
	*/
	_updateDetailListVarsChild : function(obj)
	{
		...
		for (var i = 0; i < obj.arr.length; i++) {
			...
			if (obj.arr[i].id == 'DummyAdmin') {
				if (obj.vars.flagAdmin) {
					arrayNew.push(obj.arr[i]);
				}
			...
			} else if (obj.arr[i].id == 'FlagAdmin') {
				if (!obj.vars.flagAdmin) continue;
				obj.arr[i].varsTextBtn = null;
				arrayNew.push(obj.arr[i]);
				...
				if (obj.vars.flagAdmin) continue;
				var arr = obj.vars.varsEntity;
				obj.arr[i].varsFormArea.varsTree.varsStatus.flagLinkUse = 0;
				obj.arr[i].varsFormArea.varsTree.varsStatus.flagRemoveUse = 0;
				var arrayNewArea = [];
				for (var j = 0; j < arr.length; j++) {
					var varsDetail = (Object.toJSON(obj.arr[i].varsFormArea.templateDetail)).evalJSON();
					varsDetail.id = arr[j].id;
