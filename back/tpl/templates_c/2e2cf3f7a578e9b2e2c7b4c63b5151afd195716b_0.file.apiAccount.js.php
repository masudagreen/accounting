<?php /* Smarty version 3.1.24, created on 2016-08-20 07:30:58
         compiled from "/storage/emulated/0/htdocs/rucaro/back/tpl/templates/else/core/base/js/apiAccount.js" */ ?>
<?php
/*%%SmartyHeaderCode:39743858457b80732b4deb5_98174912%%*/
if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '2e2cf3f7a578e9b2e2c7b4c63b5151afd195716b' => 
    array (
      0 => '/storage/emulated/0/htdocs/rucaro/back/tpl/templates/else/core/base/js/apiAccount.js',
      1 => 1471523676,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '39743858457b80732b4deb5_98174912',
  'variables' => 
  array (
    'varsLoad' => 0,
    'numNews' => 0,
  ...
  'version' => '3.1.24',
  'unifunc' => 'content_57b80732c17bb5_67974083',
),false);
/*/%%SmartyHeaderCode%%*/
if ($_valid && !is_callable('content_57b80732c17bb5_67974083')) {
function content_57b80732c17bb5_67974083 ($_smarty_tpl) {

$_smarty_tpl->properties['nocache_hash'] = '39743858457b80732b4deb5_98174912';
?>
 ...
 * http://rucaro.org/
 */
var Code_Core_Base_ApiAccount = Class.create(Code_Lib_ExtPortal,
{
...
,
	...
	*/
	bootAutoSearch : function(obj)
	{
		...
		} else if (str == 'id' || str == 'idAccount') {
			varsTmpl.flagType = 'num';
			varsTmpl.strColumn = str;
			varsTmpl.value = obj.vars.vars[str];
			varsData.push(varsTmpl);

		} else if (str == 'ip' || str == 'strSiteUrl') {
			varsTmpl.strColumn = str;
			varsTmpl.value = obj.vars.vars[str];
			varsData.push(varsTmpl);
		}
	...
	_getDetailChildVars : function(obj)
	{
		...
		for (var i = 0; i < obj.arr.length; i++) {
			if (obj.arr[i].id == 'Ip') {
				...
				else if (obj.flag == 'copy') obj.arr[i].value = obj.vars.vars.ip;
				else if (obj.flag == 'edit') obj.arr[i].value = obj.vars.vars.ip;
				arrayNew.push(obj.arr[i]);

			} else if (obj.arr[i].id == 'StrSiteUrl') {
				if (obj.flag == 'add') obj.arr[i].value = 'https://';
				else obj.arr[i].value = obj.vars.vars.strSiteUrl;
				arrayNew.push(obj.arr[i]);

			} else if (obj.arr[i].id == 'ArrSpaceStrTag') {
				if (obj.flag == 'add') obj.arr[i].value = '';
				else obj.arr[i].value = obj.vars.vars.arrSpaceStrTag;
				arrayNew.push(obj.arr[i]);

			} else if (obj.arr[i].id == 'IdAccount') {
				if (obj.flag == 'add') {
					...
					obj.arr[i].value = obj.vars.vars.idAccount;
					if (obj.arr[i].value) {
						var data = (Object.toJSON(obj.arr[i].varsFormList.templateDetail)).evalJSON();
						data.value = obj.vars.varsColumnDetail.idAccount;
	...
	*/
	_updateDetailListVarsChild : function(obj)
	{
		...
		for (var i = 0; i < obj.arr.length; i++) {
			...
			if (obj.arr[i].id == 'StampRegister') {
			...
			} else if (obj.arr[i].id == 'DummyIdAccount') {
				obj.arr[i].value = obj.vars.varsColumnDetail.idAccount;
				var temp = {};
				temp.id = obj.arr[i].id;
				temp.strTitle = obj.arr[i].value;
				temp.vars = {};
				temp.vars.idAccount = obj.vars.vars.idAccount;
				temp.vars.idTarget = 'idAccount';
				obj.arr[i].varsTextBtn.push(temp);
				arrayNew.push(obj.arr[i]);

			} else if (obj.arr[i].id == 'Ip') {
				obj.arr[i].value = obj.vars.varsColumnDetail.ip;
				var temp = {};
				temp.id = obj.arr[i].id;
				temp.strTitle = obj.arr[i].value;
				temp.vars = {};
				...
				obj.arr[i].value = obj.vars.varsColumnDetail.strSiteUrl;
				var temp = {};
				temp.id = obj.arr[i].id;
				temp.strTitle = obj.arr[i].value;
				temp.vars = {};
