 ...
 * http://rucaro.org/
 */
class Code_Else_Plugin_Accounting_Jpn_CalcAccountTitleFSBatch14200 extends Code_Else_Plugin_Accounting_Jpn_CalcAccountTitleBatch14200
{
	...
	protected function _loopVarsValue($arr)
	{
		...
		foreach ($array as $key => $value) {
			...
			$arr['varsFSValue'][$strFS . 'BS'][$key]['unappropriatedRetainedEarnings']['sumPrev'] = 0;
			...
			foreach ($arrayStr as $keyStr => $valueStr) {
				...
				$arr['varsFSValue'][$strFS . 'BS'][$key]['liabilitiesNetAssetsNet'][$valueStr] = $numAssetsSum;

				$numNetAssetsSum = $arr['varsFSValue'][$strFS . 'BS'][$key]['netAssetsSum'][$valueStr];
				$numliabilitiesSum = $arr['varsFSValue'][$strFS . 'BS'][$key]['liabilitiesSum'][$valueStr];
		...
		foreach ($arrayAccountTitle as $keyAccountTitle => $valueAccountTitle) {
			...
			foreach ($array as $key => $value) {
				...
				if (is_null($varsValue[$key][$idAccountTitleJgaapFS])) {
					$varsValue[$key][$idAccountTitleJgaapFS]['sumPrev'] = 0;
					$varsValue[$key][$idAccountTitleJgaapFS]['sumNext'] = 0;
				}

				$sumPrev =  $arr['varsValueAccountTitle'][$key][$idAccountTitle]['sumPrev'];
				...
				$varsValue[$key][$idAccountTitleJgaapFS]['sumPrev'] += $sumPrev;
