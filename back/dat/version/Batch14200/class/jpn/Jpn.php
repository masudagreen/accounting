 ...
 * http://rucaro.org/
 */
class Code_Else_Plugin_Accounting_Jpn_JpnBatch14200 extends Code_Else_Plugin_Accounting_AccountingBatch14200
{
	...
	{
		$arr = $this->_extendSelf;
		foreach ($arr as $key => $value) {
			if (is_array($value)) {
				continue;
			}
			$strDir = str_replace("\\", '/', $value);
			$arrDir = preg_split("/\//", $strDir);
			$strName = end($arrDir);
			if (preg_match('/^back/', $value) && preg_match('/(.*?).(.*?)$/', $strName)) {
				$this->_extendSelf[$key] = PATH_BATCH14200_VARS . $strName;
			}
		}
	}
	...
	public function run()
	{
		...
		$path = PATH_BACK_CLASS_ELSE_PLUGIN . 'accounting/' . $batch14200PLUGIN_ACCOUNTING_STR_NATION . '/' . $str . ".php";
		$strClass = 'Code_Else_Plugin_Accounting_' . ucwords($batch14200PLUGIN_ACCOUNTING_STR_NATION) . '_' . $str;
	...
	protected function _checkCorporationClass($arr)
	{
		...
		if (!$batch14200PLUGIN_ACCOUNTING_FLAG_CORPORATION) {
			return;
		}
		...
		$path = PATH_BACK_CLASS_ELSE_PLUGIN
			. 'accounting'
			. '/' . $batch14200PLUGIN_ACCOUNTING_STR_NATION
			. '/' . $str . '.php';
		...
		$path = PATH_BACK_CLASS_ELSE_PLUGIN
			. 'accounting'
			. '/' . $batch14200PLUGIN_ACCOUNTING_STR_NATION
			. '/' . $batch14200PLUGIN_ACCOUNTING_NUM_YEAR_SHEET
		...
		$strClass = 'Code_Else_Plugin_Accounting'
			. '_' . ucwords($batch14200PLUGIN_ACCOUNTING_STR_NATION)
			. '_' . $str
			. '_' . $batch14200PLUGIN_ACCOUNTING_NUM_YEAR_SHEET
			. '_' . ucwords($batch14200PLUGIN_ACCOUNTING_FLAG_CORPORATION);
	...
	protected function _setInitJpn()
	{
		...
		if ($varsEntityNation['flagCorporation'] == 2) {
			define('$batch14200PLUGIN_ACCOUNTING_FLAG_CORPORATION', 'public');
		} else {
			define('$batch14200PLUGIN_ACCOUNTING_FLAG_CORPORATION', '');
		}*/
	}
	...
	protected function _getVarsStampTerm($arr)
	{
		...
		$numTimeZone = $batch14200PLUGIN_ACCOUNTING_NUM_TIME_ZONE;
	...
	protected function _getVarsConsumptionTax($arr)
	{
		...
		$strNation = $batch14200PLUGIN_ACCOUNTING_STR_NATION;
	...
	protected function _getClass($arr)
	{
		...
		if ($arr['flagAccounting']) {
			$path = PATH_BATCH14200_CLASS . '' . $str . ".php";
		} else {
			$path = PATH_BATCH14200_CLASS . 'jpn/' . $str . ".php";
		}
		if (!file_exists($path)) {
			if (FLAG_TEST) {
				var_dump(__CLASS__ . '/' .__FUNCTION__. '/' .__LINE__);
		...
		require_once($path);
		if ($arr['flagAccounting']) {
			$strClass = 'Code_Else_Plugin_Accounting_' . $str . 'BATCH14200';
		} else {
			$strClass = 'Code_Else_Plugin_Accounting_Jpn_' . $str . 'BATCH14200';
		}
		$classCall = new $strClass;
	...
	protected function _getClassCalc($arr)
	{
		...
		if ($arr['flagDetail']) {
			...
			require_once(PATH_BATCH14200_CLASS . $strNation . '/' . $strDir . '/' . $strFile .'.php');
			if ($batch14200PLUGIN_ACCOUNTING_FLAG_CORPORATION) {
				$path = PATH_BATCH14200_CLASS . $strNation . '/' . $batch14200PLUGIN_ACCOUNTING_NUM_YEAR_SHEET . '/' . $batch14200PLUGIN_ACCOUNTING_FLAG_CORPORATION  . '/' . $strDir . '/' . $strFile .'.php';
				if (!file_exists($path)) {
					$flag = 1;
				} else {
					require_once($path);
					$strClass = 'Code_Else_Plugin_Accounting_' . ucwords($strNation) . '_' . ucwords($strDir) .'_' . $strFile .'_' . $batch14200PLUGIN_ACCOUNTING_NUM_YEAR_SHEET .'_' . ucwords($batch14200PLUGIN_ACCOUNTING_FLAG_CORPORATION) . 'BATCH14200';
				}
			} else {
				$flag = 1;
			}
			if ($flag) {
				...
				$strClass = 'Code_Else_Plugin_Accounting_' . $strNation .'_' . $strDir .'_' . $strFile . 'BATCH14200';
			}

		} else {
			require_once(PATH_BATCH14200_CLASS . $strNation . '/' . 'Calc' . $arr['flagType'] . '.php');
			if ($batch14200PLUGIN_ACCOUNTING_FLAG_CORPORATION) {
				$strFile = 'Calc' . $arr['flagType'];
				$path = PATH_BATCH14200_CLASS . $strNation . '/' . $batch14200PLUGIN_ACCOUNTING_NUM_YEAR_SHEET . '/' . $batch14200PLUGIN_ACCOUNTING_FLAG_CORPORATION  . '/' . $strFile .'.php';
				if (!file_exists($path)) {
					$flag = 1;
				} else {
					require_once($path);
					$strNation = ucwords($strNation);
					$strClass = 'Code_Else_Plugin_Accounting_' . ucwords($strNation) . '_' . $strFile . '_' . $batch14200PLUGIN_ACCOUNTING_NUM_YEAR_SHEET . '_' . ucwords($batch14200PLUGIN_ACCOUNTING_FLAG_CORPORATION) . 'BATCH14200';
				}
			} else {
				$flag = 1;
			}
			if ($flag) {
				$strNation = ucwords($strNation);
				$strClass = 'Code_Else_Plugin_Accounting_' . $strNation .'_' . 'Calc' . $arr['flagType'] . 'BATCH14200';
			}
		}
	...
	protected function _setClassExt($arr)
	{
		...
		if ($batch14200PLUGIN_ACCOUNTING_FLAG_CORPORATION) {
			$path = PATH_BACK_CLASS_ELSE_PLUGIN . 'accounting/jpn/' . $batch14200PLUGIN_ACCOUNTING_NUM_YEAR_SHEET . '/' . $batch14200PLUGIN_ACCOUNTING_FLAG_CORPORATION  . '/' . $str .'.php';
			if (!file_exists($path)) {
				$flag = 1;
			} else {
				require_once($path);
				$strClass = 'Code_Else_Plugin_Accounting_Jpn_' . $str .'_' . $batch14200PLUGIN_ACCOUNTING_NUM_YEAR_SHEET .'_' . ucwords($batch14200PLUGIN_ACCOUNTING_FLAG_CORPORATION);
			}
		} else {
			$flag = 1;
		}
