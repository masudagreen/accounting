 ...
 * http://rucaro.org/
 */
class Code_Else_Plugin_Accounting_AccountingBatch14200 extends Code_Else_Core_Base_ModuleAbstract
{
	...
	function __construct()
	{
		...
		foreach ($arr as $key => $value) {
			...
			$strName = end($arrDir);
			if (preg_match('/^back\/tpl/', $value) && preg_match('/(.*?).(.*?)$/', $strName)) {
				$this->_self[$key] = PATH_BATCH14200_VARS . $strName;
			}
		}
	}
	...
	public function run()
	{
		...
		if ($varsRequest['query']['ext']) {
			...
			if ($str == 'Entity'
				...
				$path = PATH_BACK_CLASS_ELSE_PLUGIN . 'accounting/' . $batch14200PLUGIN_ACCOUNTING_STR_NATION . '/' . ucwords($batch14200PLUGIN_ACCOUNTING_STR_NATION) . ".php";
				$strClass = 'Code_Else_Plugin_Accounting_' . ucwords($batch14200PLUGIN_ACCOUNTING_STR_NATION) . '_' . ucwords($batch14200PLUGIN_ACCOUNTING_STR_NATION);
	...
	protected function _setInitNation()
	{
		...
		global $batch14200PLUGIN_ACCOUNTING_STR_NATION;
		$batch14200PLUGIN_ACCOUNTING_STR_NATION = $strNation;
	}
	...
	protected function _getClassNation($arr)
	{
		...
		require_once($path);
		$strClass = 'Code_Else_Plugin_Accounting_' . $batch14200PLUGIN_ACCOUNTING_STR_NATION  . '_' . $str;
		$classCall = new $strClass;
	...
	protected function _setInitLang()
	{
		...
		global $batch14200PLUGIN_ACCOUNTING_STR_LANG;
		$batch14200PLUGIN_ACCOUNTING_STR_LANG = $strLang;
	}
