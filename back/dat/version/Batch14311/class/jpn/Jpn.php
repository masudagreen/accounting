 ...
 * http://rucaro.org/
 */
class Code_Else_Plugin_Accounting_Jpn_JpnBatch14311 extends Code_Else_Plugin_Accounting_AccountingBatch14311
{
	...
	function __construct()
	{
		...
		foreach ($arr as $key => $value) {
			...
			if (preg_match('/^back/', $value) && preg_match('/(.*?).(.*?)$/', $strName)) {
				$this->_extendSelf[$key] = PATH_BATCH14311_VARS . $strName;
			}
		}
	}
	...
	public function run()
	{
		...
		$strClass = 'Code_Else_Plugin_Accounting_' . ucwords('jpn') . '_' . $str;
	...
	protected function _getClass($arr)
	{
		...
		if ($arr['flagAccounting']) {
			$path = PATH_BATCH14311_CLASS . $str . ".php";
		} else {
			$path = PATH_BATCH14311_CLASS . 'jpn/' . $str . ".php";
		}
		if (!file_exists($path)) {
			if (FLAG_TEST) {
				var_dump(__CLASS__ . '/' .__FUNCTION__. '/' .__LINE__);
		...
		require_once($path);
		if ($arr['flagAccounting']) {
			$strClass = 'Code_Else_Plugin_Accounting_' . $str . 'Batch14311';
		} else {
			$strClass = 'Code_Else_Plugin_Accounting_Jpn_' . $str . 'Batch14311';
		}
		$classCall = new $strClass;
	...
	protected function _getClassCalc($arr)
	{
		...
		if ($arr['flagDetail']) {
			...
			require_once(PATH_BATCH14311_CLASS . $strNation . '/' . 'Calc' . $arr['flagType'] . '.php');
			$strFile = 'Calc' . $arr['flagType'];
			$path = PATH_BATCH14311_CLASS . $this->_self['strTitle'] . '/' . $strNation . '/' . '2012' . '/' . 'public'  . '/' . $strFile .'.php';
			if (!file_exists($path)) {
				$flag = 1;
			} else {
				require_once($path);
				$strNation = ucwords($strNation);
				$strClass = 'Code_Else_Plugin_Accounting_' . ucwords($strNation) . '_' . $strFile . '_' . '2012' . '_' . ucwords('public') . 'Batch14311';
			}
			if ($flag) {
				$strNation = ucwords($strNation);
				$strClass = 'Code_Else_Plugin_Accounting_' . $strNation .'_' . 'Calc' . $arr['flagType'] . 'Batch14311';
			}
		}
	...
	protected function _getClassCalc($arr)
	{
		...
		require_once(PATH_BATCH14311_CLASS . $strNation . '/' . 'Calc' . $arr['flagType'] . '.php');
		$strFile = 'Calc' . $arr['flagType'];
		$path = PATH_BATCH14311_CLASS . $strNation . '/' . '2012' . '/' . 'public'  . '/' . $strFile .'.php';
		require_once($path);
		$strNation = ucwords($strNation);
		$strClass = 'Code_Else_Plugin_Accounting_' . ucwords($strNation) . '_' . $strFile . '_' . '2012' . '_' . ucwords('public') . 'Batch14311';
