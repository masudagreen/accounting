...
final readonly class FixedAsset
{
    public function __construct(
        ...
        if ($acquisitionDate > $serviceStartDate) {
            throw ValidationException::withErrors(['serviceStartDate' => ['serviceStartDate must be on or after acquisitionDate.']]);
        }
        if ($disposalDate !== null && $disposalDate < $serviceStartDate) {
            throw ValidationException::withErrors(['disposalDate' => ['disposalDate must be on or after serviceStartDate.']]);
        }
        if (Decimal::compare($acquisitionCost, '0.0000') <= 0) {
            throw ValidationException::withErrors(['acquisitionCost' => ['acquisitionCost must be > 0.']]);
        }
        if (Decimal::compare($residualValue, '0.0000') < 0) {
            throw ValidationException::withErrors(['residualValue' => ['residualValue must be >= 0.']]);
        }
        if (Decimal::compare($residualValue, $acquisitionCost) > 0) {
            throw ValidationException::withErrors(['residualValue' => ['residualValue must be <= acquisitionCost.']]);
        }
        if ($usefulLifeYears < 0) {
            throw ValidationException::withErrors(['usefulLifeYears' => ['usefulLifeYears must be >= 0.']]);
        }
        if ($quantity < 1) {
            throw ValidationException::withErrors(['quantity' => ['quantity must be >= 1.']]);
        }
    }
