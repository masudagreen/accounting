...
{block name="content"}
  <header class="mb-4">
    <h1 class="h3 mb-1 text-danger">
      <i class="bi bi-exclamation-triangle-fill"></i> 勘定科目の削除確認
    </h1>
  ...
  <div class="rucaro-card p-4 shadow-sm border-danger">
    <dl class="row g-2 mb-4">
      ...
      <dt class="col-sm-3 text-muted">分類</dt>
      <dd class="col-sm-9">{$category_labels[$target.category]|default:$target.category|escape}</dd>
    </dl>

    <div class="alert alert-danger small mb-3">
      <i class="bi bi-exclamation-circle-fill"></i>
      この勘定科目を <strong>本当に削除</strong> しますか？ 仕訳で使用中の場合は削除できません。
    </div>

    <form method="post" action="/ui/masters/account-titles/{$target.id|escape}/delete" class="d-flex gap-2 justify-content-end">
