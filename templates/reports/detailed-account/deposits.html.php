 ...
 * 預貯金等の内訳書
 *
 * @var \App\Domain\Report\DetailedAccount\DepositsBreakdown $data
 */
...
<h1 class="report-title">預貯金等の内訳書</h1>
...
<table class="report-table">
    <thead>
        <tr>
            <th class="label" style="width:30%">金融機関名・支店名</th>
            <th style="width:20%">所在地</th>
            <th style="width:20%">種類・口座番号</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><?= HtmlHelper::e($row->note) ?></td>
            <td class="amount"><?= HtmlHelper::e(HtmlHelper::money($row->closingBalance)) ?></td>
        </tr>
        <?php endforeach; ?>
        <?php if ($data->rows === []): ?>
        <tr>
            <td class="label" colspan="4" style="text-align:center">（該当なし）</td>
        </tr>
        <?php endif; ?>
    </tbody>
    <tfoot>
        <tr class="total">
            <td class="label" colspan="3">合計</td>
