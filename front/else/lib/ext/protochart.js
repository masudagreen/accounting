...
Proto.Chart = Class.create({
	 ...
	 * {Object} options
	 */
	initialize: function(elem, data, options, eleScroll, insRoot)
	{
		options = options || {};
		...
		this.options = this.merge(options,{
			colors: ["#0068B7", "#E60012", "#F39800", "#009944", "#FFF100", "#00A0E9", "#E5004F", "#009E96", "#1D2088", "#920783", "#009E96", "#8FC31F"],
            legend: {
                show: false,
                noColumns: 1,
                labelFormatter: null,
	...
	*/
	...
	{
		var array = this.domObj.style.width.split('px');
	...
	*/
	...
	{
		var array = this.domObj.style.height.split('px');
		var data = parseFloat(array[0]);
	...
	constructCanvas: function() {
		...
		*/
		this.canvasWidth = this._getWrapWidth();
		this.canvasHeight = this._getWrapHeight();
	...
	prepareTickGeneration: function(axis, axisOptions) {
		...
		if (axisOptions.mode == "time") {
			...
			generator = function(axis) {
				...
				do {
					v = d.getTime();
					ticks.push({ v: v, label: axis.tickFormatter(v, axis, axisOptions) });
					if (unit == "month") {
						if (tickSize < 1) {
							d.setDate(1);
							var start = d.getTime();
			...
			formatter = function (v, axis, axisOptions) {
				if(v) {
					v = v.toFixed(axis.tickDecimals);
					var num = v;
					if (axisOptions.comma) {
						num = new String(v).replace(/,/g, "");
						while(num != (num = num.replace(/^(-?\d+)(\d{3})/, "$1,$2")));
					}
					v = num + axisOptions.unit;
					return v;
				}
				return 0;
			};
		...
		axis.tickSize = unit ? [size, unit] : size;//1
		axis.tickGenerator = generator;
		if (Object.isFunction(axisOptions.tickFormatter)) {
			axis.tickFormatter = function (v, axis) { return "" + axisOptions.tickFormatter(v, axis, axisOptions); };
		} else {
	...
	setTicks: function(axis, axisOptions) {
		...
		if (axisOptions.ticks == null)
			...
			ticks.each(function(t, i){
				...
				if(!label) {
					label = axis.tickFormatter(v, axis, axisOptions);
				}
	...
	setSpacing: function() {
		...
		if (labels.length > 0) {
			 ...
			 * */
			$(this.insRoot.vars.varsSystem.id.root).insert(dummyDiv);
	...
	drawPieShadow: function(anchorX, anchorY, radius)
	{
		...
		this.context.arc(anchorX + 0, anchorY + 0, radius + (this.options.shadowSize/2), startAngle, endAngle, false);
		this.context.fill();
	...
	drawPieGraph: function(graphData)
	{
		...
		graphData.each(function(gd, j){
			...
			var textVal = this.getPointNumber(pct*100, 3);
			gd.textVal = '';
			if (textVal === '') {
				return true;
			}
...
*/
			var num = gd.data;
			num += this.options.pies.unit;

			var strTitle = gd.label + ' : ' + num;

			gd.textVal = textVal;

			var html = "<div title=\"" + strTitle + "\" style=\"position: absolute;zindex:11; width:" + labelWidth + "px;fontSize:" + fontSize + "px;overflow:hidden;top:"+ top + ";left:"+ left + ";textAlign:" + textAlign + ";verticalAlign:" + verticalAlign +"\">" +  textVal + "%</div>";
	...
	getPointNumber: function(num, numPoint)
	{
		...
		var str = new String(Math.floor(num * Math.pow(10, numPoint)) / Math.pow(10, numPoint));
		if (str.indexOf('.') < 0) {
			str += '.';
		}
	...
	onClick: function(e) {
		...
		var numScrollLeft = 0;
		if (this.eleScroll) {
			numScrollTop = this.eleScroll.scrollTop;
			numScrollLeft = this.eleScroll.scrollLeft;
		}

		var offset = this.overlay.cumulativeOffset();
		var pos ={
			x: this.xaxis.min + (e.pageX - offset.left - this.chartOffset.left + numScrollLeft) / this.hozScale,
			y: this.yaxis.max - (e.pageY - offset.top - this.chartOffset.top + numScrollTop) / this.vertScale
		};
		...
		if(this.options.allowDataClick)
		{
			...
			else if(this.options.bars.show)
			{
				if(this.barDataRange.length > 0)
				{
					...
					if (!Prototype.Browser.IE) {
						if (dataPoint.value !== '') {
							var id = dataPoint.indexLabel + '_' + dataPoint.indexData;
							var num = dataPoint.value;
							if (this.options.yaxis.comma) {
								num = new String(num).replace(/,/g, "");
								while(num != (num = num.replace(/^(-?\d+)(\d{3})/, "$1,$2")));
							}
							num += this.options.yaxis.unit;
							var strLabel = this.graphData[dataPoint.indexLabel].label;
							var strData = this.options.xaxis.ticks[dataPoint.indexData][1];
							var strDataTitle = this.options.xaxis.ticks[dataPoint.indexData][2];
							if (strDataTitle != undefined) {
								strData = strDataTitle;
							}
							var str = strLabel + '<br>X : ' + strData + ' ,' + 'Y : ' + num + '';

							var ele = $(document.createElement('span'));
							ele.insert(str);
							this.domObj.insert(ele);
							var numWidth = ele.offsetWidth;
							var numTop = e.pageY - offset.top - this.chartOffset.top;
							var numLeft = e.pageX - offset.left - this.chartOffset.left;
							if (this.eleScroll) {
								numTop += this.eleScroll.scrollTop;
								numLeft += this.eleScroll.scrollLeft;
							}
							ele.setStyle({
								position        : 'absolute',
								opacity         : this.options.pop.backgroundOpacity,
								backgroundColor : this.options.pop.backgroundColor,
								border          : this.options.pop.border,
								padding         : '2px',
								width           : numWidth,
								textAlign       : 'center',
								fontSize        : 'smaller',
							...
							if (this._objPop[id]) {
								var arrNew = [];
								for (var i = 0; i < this._arrPop.length; i++) {
									if (id == this._arrPop[i].id) {
										if (this._arrPop[i].ele.parentNode) {
											this._arrPop[i].ele.remove();
										}
									} else {
										arrNew.push(this._arrPop[i]);
									}
								}
								this._arrPop = arrNew;
							}
							var tempData = {id : id, ele: ele};
							this._arrPop.push(tempData);
							this._objPop[id] = 1;
	...
	getDataClickPoint: function(pos, options, barDataRange)
	{
		...
		{
			this.barDataRange.each(function(barData, i){
				barData.each(function(data, j){
					var temp = data;
					if(xClick > temp.left && xClick < temp.right) {
						if (yClick >= 0) {

							if(yClick < temp.value) {
								dataVal.value = temp.value;
								dataVal.indexLabel = i;
								dataVal.indexData = parseInt(pos.x.toFixed(0));
								throw $break;
							}
						} else {
